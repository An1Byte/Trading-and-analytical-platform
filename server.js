...
let List = ""; // Ïåðåìåííàÿ, êîòîðàÿ õðàíèò â ñåáå âåñü òåêñò âñåõ ôàéëîâ List.txt
let tempList = "";
...
function recurrentReading(n, max) {
    return new Promise((resolve, reject) => {
        if (n < max) {
            ...
            // Ñ÷èòûâàåì ôàéëû ñ èñòîðèåé ïî èíñòðóìåíòó ïîä èíäåêñîì n:
...
function start_GUI() {
    ...
    app.get("/getList", function (req, res) {
        ...
        if (List != "") {
            // Îïðåäåëÿåì ñïèñîê èíñòðóìåíòîâ:
            let temp = "";
            ...
            if (Files_wasBeEqual == false) {
                // ------------------- Ñîáèðàåì èñòîðèþ ïî âñåì èíñòðóìåíòàì:
...
// Ôóíêöèÿ, êîòîðàÿ áóäåò çàâåðøàòü ðàáîòó ñêðèïòà â íîðìàëüíîì  è àâàðèéíîì ñîñòîÿíèè:
function QuitThisServer() {
    fs.exists("./Files/List.txt", (bf) => {
        if (bf) {
            ...
            process.exit(1); // Âñòðîåííûé ìîäóëü process çàâåðøàåò ðàáîòó ñåðâåðà ïðåæäåâðåìåííî.
        }
    });
}

// Ôóíêöèÿ ðåêóðñèâíîãî ñ÷èòûâàíèÿ ôàéëîâ List.txt ïî çàäàííûì ïóòÿì:
...
fs.exists("./Files/Paths_to_Invizzz.txt", (bF) => { // Àñèíõðîííàÿ ïðîâåðêà ôàéëà íà ïðåäìåò åãî ñóùåñòâîâàíèÿ... 
        if (!bF) { // Åñëè ôàéë íå ñóùåñòâóåò, òî åãî íåîáõîäèìî ñîçäàòü è çàâåðøèòü ðàáîòó ñåðâåðà.
            ...
            // Ñ÷èòûâàåì ôàéë Paths_to_Invizzz.txt:
            fs.readFile("./Files/Paths_to_Invizzz.txt", "utf8", (err, datas) => {
                if (err) {
                    ...
                    let arrayPaths = []; // Àáñîëþòíûå ïóòè ê ïàïêå Invizzz
                    ...
                    console.log(arrayPaths[1]);
                        start_GUI(); // Çàïóñêàåì ñåðâåð.

                        setInterval(() => { // Òàéìåð äëÿ ïåðåçàïèñè ìàññèâà ñ èíñòðóìåíòàìè List...
                            tempList = "";
